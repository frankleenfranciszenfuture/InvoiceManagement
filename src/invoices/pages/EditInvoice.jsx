...
export default function EditInvoice() {
    ...
    const salesPersonName = useSelector(
        (state) => state.invoice.salesPersonName
    );

    const salesPersonId = useSelector(
        (state) => state.invoice.salesPersonId
    );
    ...
    //customer edit useEffect load
    useEffect(() => {
        if (!id || customers.length === 0 || !invoice.customerId) return;
        ...
        if (customer) {
            ...
            dispatch(
                updateInvoiceField({
                    field: "currency",
                    value: customer.currency || "INR",
                })
            );

            dispatch(setCurrencySearch(customer.currency || "INR"));
        }
    }, [id, customers, invoice.customerId, dispatch]);


    //salesPerson edit useEffect load
    useEffect(() => {
        if (!salesPersonId || salesPerson.length === 0) return;
        ...
        dispatch(setSalesPersonName(selected.salesPersonName));
        dispatch(setSalesPersonSearch(selected.salesPersonName));
    }, [salesPersonId, salesPerson, dispatch]);
    ...
    const handleSave = async (invoiceStatus) => {
        ...
        const payload = {
            ...
            items: invoice.invoiceItems.map(item => ({
                id: item.id, // include if your backend supports item updates
                itemId: item.itemId,
        ...
        try {
            ...
            if (invoice.id) {
                console.log("UPDATE API CALLED", invoice.id);

                savedInvoice = await dispatch(
                    editInvoice({
                        id: invoice.id,
                        data: payload,
                    })
                ...
                savedInvoice = await dispatch(addInvoice(payload)).unwrap();
            }

            dispatch(resetDirty());
            dispatch(resetInvoice());

            const successMessages = invoice.id
                ? {
                    ACTIVE: "Invoice updated successfully.",
                    DRAFT: "Draft updated successfully.",
                    INACTIVE: "Invoice updated successfully.",
                }
                : {
            ...
            const errorMessages = invoice.id
                ? {
                    ACTIVE: "Failed to update invoice.",
                    DRAFT: "Failed to update draft.",
                    INACTIVE: "Failed to update invoice.",
                }
                : {
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    >
                                        <input
                                            type="text"
                                            value={openCustomer ? customerSearch : (customerName || customerSearch)}
                                            placeholder="Select or add a customer"
                                            onClick={() => {
                                                dispatch(setOpenCustomer(!openCustomer));

                                                if (!openCustomer) {
                                                    dispatch(setCustomerSearch(customerName || ""));
                                                }
                                            }}
                                            onChange={(e) =>
                                ...
                                <div ref={salesPersonDropdownRef} className="relative flex-1">
                                    <input
                                        ...
                                        onClick={() => {
                                            ...
                                            if (!openSalesPerson) {
                                                dispatch(setSalesPersonSearch(salesPersonName));
                                            }
                                        }}
                                        onChange={(e) =>
