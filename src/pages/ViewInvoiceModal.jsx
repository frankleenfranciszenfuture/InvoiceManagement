...
export default function ViewInvoiceModal({ invoice: inv }) {
  const invoice = useSelector((state) => state.invoice.selectedInvoice);

  if (!invoice) return <h1>Invoice not found</h1>; // ✅ check first

  return <InvoicePreview invoice={invoice} />;

  const handle = async (status) => {
    try {
      await dispatch(changeInvoiceStatus({ id: inv.id, status })).unwrap();
  ...
  return (
    <div className="px-9 pt-10 pb-6 border border-gray-200">
      ...
      {/* <div className="border-t border-gray-100 pt-4 space-y-1 text-sm text-right mb-6">
        <div className="flex justify-between text-gray-500">
          <span>Subtotal</span>
          <span>{fmt.currency(invoice.subtotal)}</span>
        </div>
        <div className="flex justify-between text-gray-500">
          <span>Discount</span>
          <span>-{fmt.currency(invoice.totalDiscount)}</span>
        </div>
        <div className="flex justify-between text-gray-500">
          <span>Tax</span>
          <span>{fmt.currency(invoice.totalTax)}</span>
        </div>
        <div className="flex justify-between text-base font-semibold text-gray-900 pt-2 border-t border-gray-100">
          <span>Total</span>
          <span>{fmt.currency(invoice.totalAmount)}</span>
        </div>
      </div>
      <div className="flex gap-2 justify-end">
        {ACTIONS.filter((a) => a.show.includes(invoice.status)).map((a) => (
          <button
            key={a.status}
            onClick={() => handle(a.status)}
            className={a.status === "PAID" ? "btn-primary" : "btn-secondary"}
          >
            {a.label}
          </button>
        ))}
