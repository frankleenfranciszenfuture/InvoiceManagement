...
export default function NavbarInvoice() {
  ...
  const { selectedInvoiceView, views, search, status } = useSelector(
  ...
  return (
    <div className="h-[60px] border border-gray-100 rounded-md  flex items-center justify-between bg-white px-2 py-2 border-b ">
      ...
      <div ref={dropdownOpenRef} className="relative">
        ...
        >
          <h2 className="font-semibold">{selectedInvoiceView}</h2>
          <ChevronDown
            size={14}
            ...
            <div className="max-h-72 overflow-y-auto">
              {filteredViews.map((view) => (
                <button
                  key={view.value}
                  onClick={() => {
                    dispatch(setSelectedInvoiceView(view.label));
